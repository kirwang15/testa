...
test("Web exposes all courses and the assessment interaction", async ({ page }, testInfo) => {
  ...
  await onboard(page);
  await expect(page.getByText("3–5 分钟测词汇量", { exact: true })).toBeVisible();
  ...
  await page.goto("/");
  await page.getByRole("link", { name: /3–5 分钟测词汇量/ }).click();
  ...
  await expect(page.getByRole("button", { name: "认识", exact: true })).toHaveClass(/bg-emerald-600/);
  await page.screenshot({ path: testInfo.outputPath("assessment-correct-green.png"), fullPage: true });

  await expect(word).not.toHaveText(firstWord, { timeout: 3_000 });
  await page.getByRole("button", { name: "不确定", exact: true }).click();
  await expect(page.getByText(/回答错误 · 正确答案：认识/)).toBeVisible();
  await expect(page.getByRole("button", { name: "不确定", exact: true })).toHaveClass(/bg-red-600/);
  await page.screenshot({ path: testInfo.outputPath("assessment-wrong-red.png"), fullPage: true });

  const answeredWord = (await word.textContent())!.trim();
  await expect(word).not.toHaveText(answeredWord, { timeout: 3_000 });
